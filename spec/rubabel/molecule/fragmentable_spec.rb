...
describe Rubabel::Molecule::Fragmentable do
  ...
  describe 'fragmentation rules' do
    ...
    describe 'oxe: oxygen electron stealing' do
      ...
      describe 'ether to ions (same for esters)' do
        mol = Rubabel["CCOCCN"]
        ...
        frags.first.exact_mass.should == 29.03912516
        frags.last.exact_mass.should == 61.052763849
      ...
      describe 'phosphodiester' do
        ...
        ff.first.exact_mass.should == 59.049689844
        ff.last.formula.should == 'C2H7NO3P'
        ff.last.exact_mass.should == 124.016354719

        mol = Rubabel["CCCOP(=O)(OCC[N+](C)(C)C)[O-]"]
        frag_set = mol.fragment(rules: [:oxe_phde, :oxe])
        frag_set.flat_map {|x| x.flat_map &:csmiles }.should ==  ["CC[CH2+]", "[O-]P(=O)(OCC[N+](C)(C)C)[O-]", "CCCOP(=O)([O-])[O-]", "[CH2+]C[N+](C)(C)C", "[O-]CCC", "O=[P](=O)OCC[N+](C)(C)C", "CCCO[P](=O)=O", "[O-]CC[N+](C)(C)C"]
          #["CC[CH2+]", "[O-]P(=O)(OCC[N+](C)(C)C)[O-]", "CCCOP(=O)([O-])[O-]", "[CH2+]C[N+](C)(C)C", "[O-]CCC", "C[N+](CCO[P+](=O)[O-])(C)C", "[NH3+]CCO[P](=O)=O", "[O-]CC[N+](C)(C)C"]
      end
    end
